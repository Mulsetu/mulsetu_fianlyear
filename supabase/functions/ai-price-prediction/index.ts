...
serve(async (req: Request): Promise<Response> => {
  ...
  const modelServerUrl = Deno.env.get("MODEL_SERVER_URL");
  if (modelServerUrl) {
    try {
      const modelResp = await fetch(`${modelServerUrl.replace(/\/+$/,'')}/predict`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ crop, mandi, horizon, candidateMandis }),
      });

      if (modelResp.ok) {
        const modelJson = await modelResp.json();
        // If the external server returns a valid structured prediction, use it
        if (modelJson && Array.isArray(modelJson.predictions) && modelJson.predictions.length > 0) {
          const parsed = parseModelPrediction(JSON.stringify(modelJson)) ?? null;
          const contextPredictions = normalizePredictionsByContext(parsed?.predictions ?? modelJson.predictions, crop, mandi || "Your selected mandi");
          const outputPredictions = horizon === "1D" ? contextPredictions.slice(0, 1) : contextPredictions;
          const rawNearbyMandis = (parsed?.nearbyMandis && parsed.nearbyMandis.length > 0) ? parsed.nearbyMandis : (modelJson.nearbyMandis || buildFallbackNearbyMandis(mandi || "Your selected mandi", contextPredictions[0].price, candidateMandis));
          const outputNearbyMandis = enforceCandidateMandis(rawNearbyMandis, candidateMandis, mandi || "Your selected mandi", contextPredictions[0].price);

          return new Response(
            JSON.stringify({
              predictions: outputPredictions,
              baseMandi: parsed?.baseMandi || modelJson.baseMandi || mandi || "Your selected mandi",
              nearbyMandis: outputNearbyMandis,
              confidence: modelJson.confidence || parsed?.confidence || "medium",
              model: modelJson.model || "external-model-server",
              summary: modelJson.summary || parsed?.summary || "Forecast generated by external model server.",
            }),
      ...
      // If external model server fails, log and continue to Gemini flow
      console.warn("Model server request failed:", err);
    }
  }
  if (!geminiApiKey) {
